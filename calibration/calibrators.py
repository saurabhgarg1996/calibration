...
class TempScaling:
    ...
    def fit(self, logits, labels, eps=1e-12):
        ...
        while (np.abs(loss - new_loss) > 1e-4):
            ...
            with torch.no_grad():
                new_loss = nll_criterion(self.temperature_scale(
                    torch_logits), torch_labels).cpu().numpy()
    ...
    def calibrate(self, logits, eps=1e-12):
        ...
        torch_logits = torch.from_numpy(logits).float().to(self.device)
        torch_logits = self.temperature_scale(
...
class VectorScaling:
    ...
    def __init__(self, num_label, bias=False, weights=None, device=None, print_verbose=False):
        ...
        self.temperature = nn.Parameter(
            torch.ones(num_label).to(self.device) * 1.5)
    ...
    def fit(self, logits, labels, eps=1e-12):
        ...
        if self.print_verbose:
            print('Optimal temperature: ',
                  self.temperature.detach().cpu().numpy())
            if self.biasFlag:
...
class MatrixScaling:
    ...
    def fit(self, logits, labels, eps=1e-12):
        ...
        def eval():
            optimizer.zero_grad()
            l2 = sum(p.square().sum()
                     for p in list(self.temperature_2.parameters()))
            loss = nll_criterion(self.temperature_scale(
